...
namespace Unity.Coding.Editor
{
    ...
    static class EditorIntegration
    {
        ...
        {
            // Due to a bug that freezes Unity when a new script is added we temporarily skip processing newly added files
            // https://github.com/Unity-Technologies/com.unity.coding/issues/24
            changedAssets = changedAssets.Where(a => !addedAssets.Contains(a)).ToArray();

            var formatContext = Utility.CreateFormatContext(FormatTriggerKind.Automatic);

            var pathsToFormat = changedAssets
    ...
    static class Utility
    {
        ...
        public static void CollectUnityAssetPathsRecursively(string path, [NotNull] List<string> assetPaths)
        {
            ...
            {
                var assets = AssetDatabase.FindAssets("*", new string[] { nPath.ToString(SlashMode.Forward) });
        ...
        public static FormatContext CreateFormatContext(FormatTriggerKind triggerKind)
        {
            return new FormatContext
            {
                ...
                // Uncomment this to enable error logging
